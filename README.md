...
`suffix` | suffix
`charge_1` | The first charge listed isn't necessarily meaningful, but mcdc & prcdf include the MS code section for the top charge, which is then parsed into `charge_1 statute` and `charge_1 title`
`charge_1 statute` | the code section for the top charge for mcdc & prcdf intakes
`charge_1 title` | the title of the top charge
